...
export async function performRoll(
  ...
  await recordRoll(token, { label, detail, total: r.total, requestId });
  return r;
}

/** Roll an arbitrary dice expression (e.g. "2d6+1") and record it. */
export async function performDice(token: string, expression: string) {
  const r = roll(expression);
  const modPart = r.modifier ? ` ${formatMod(r.modifier)}` : '';
  const detail = `[${r.rolls.join(', ')}]${modPart}`;
  await recordRoll(token, { label: `Dadi ${expression}`, detail, total: r.total });
  return r;
}
